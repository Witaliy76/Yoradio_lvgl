...
constexpr uint16_t kMinSrcSide = 480u;
...
static bool s_prepare_jpeg(const char* fs_path, uint16_t dst_w, uint16_t dst_h,
    ...
    if (!f) {
        Serial.printf("[MAIN_BG] open fail %s\n", fs_path);
        return false;
    }
    const size_t file_sz = static_cast<size_t>(f.size());
    uint8_t* jpeg = static_cast<uint8_t*>(ps_malloc(file_sz));
    if (!jpeg) {
        f.close();
    ...
    if (jr != JDR_OK) {
        Serial.printf("[MAIN_BG] jd_prepare rc=%d %s\n", (int)jr, fs_path);
        free(pool);
        free(jpeg);
        return false;
    }
    if (jd.width < kMinSrcSide || jd.height < kMinSrcSide) {
        Serial.printf("[MAIN_BG] geometry %ux%u too small\n", (unsigned)jd.width, (unsigned)jd.height);
        free(pool);
        free(jpeg);
        return false;
    }
    ...
    out_buf = dst;
    Serial.printf("[MAIN_BG] ready %s src=%ux%u dst=%ux%u payload=%u\n",
                  fs_path, (unsigned)sink.src_w, (unsigned)sink.src_h,
                  (unsigned)dst_w, (unsigned)dst_h, (unsigned)dst_bytes);
    return true;
}
...
static void s_worker_task(void* /*arg*/) {
    for (;;) {
        ...
        uint8_t* buf = nullptr;
        lv_img_dsc_t dsc = {};
        const bool ok = s_prepare_jpeg(job.path, job.dst_w, job.dst_h, job.fill, job.gen, buf, dsc);
        if (!ok || s_job_stale(job.gen)) {
            if (buf) free(buf);
            // Failed current job: WebUI must not stay on Loading. Stale job leaves busy for the newer one.
            // Сбой текущего job: WebUI не должен застрять на Loading. Устаревший job busy не снимает.
            portENTER_CRITICAL(&s_mux);
        ...
        s_done_buf = buf;
        s_done_dsc = dsc;
        strlcpy(s_done_path, job.path, sizeof(s_done_path));
        s_done_gen = job.gen;
        s_done_ready = true;
        portEXIT_CRITICAL(&s_mux);
    }
...
bool mainBgCachePreloadActive() {
    ...
    const char* path = mainBgJpegPathForSlot(slot);
    ...
    uint8_t* buf = nullptr;
    lv_img_dsc_t dsc = {};
    // gen=s_req_gen so a concurrent runtime job cannot abort boot (req_gen is 0 until first async).
    const bool ok = s_prepare_jpeg(path, dst_w, dst_h, fill, s_req_gen, buf, dsc);
...
void mainBgCacheRequestAsync(const char* fs_path) {
    ...
    strlcpy(s_wanted_path, fs_path, sizeof(s_wanted_path));
    s_req.dst_w = dst_w;
    s_req.dst_h = dst_h;
    s_req.fill  = fill;
    s_req.gen   = s_req_gen;
...
bool mainBgCachePollApply() {
    ...
    s_done_path[0] = '\0';
    s_done_ready = false;
    const bool stale = (gen != s_req_gen) || (strncmp(path, s_wanted_path, sizeof(path) - 1) != 0);
    portEXIT_CRITICAL(&s_mux);
