...
size_t Audio::resampleTo48kStereo(const int16_t* input, size_t inputSamples) {
    ...
    auto clipToInt16 = [this](float value) -> int16_t {
        if (value > 32767.0f){AUDIO_INFO("overflow +"); return 32767;}
    ...
    switch(codec) {
        case CODEC_MP3:
            if(!MP3Decoder_IsInit()){
                if(!MP3Decoder_AllocateBuffers()) {
                    AUDIO_ERROR("The MP3Decoder could not be initialized");
                    goto exit;
                }
        ...
        case CODEC_AAC:
            if(!AACDecoder_IsInit()) {
                if(!AACDecoder_AllocateBuffers()) {
                    AUDIO_ERROR("The AACDecoder could not be initialized");
                    goto exit;
                }
        ...
        case CODEC_FLAC:
            if(!FLACDecoder_AllocateBuffers()) {
                AUDIO_ERROR("The FLACDecoder could not be initialized");
                goto exit;
            }
        ...
        case CODEC_OPUS:
            if(!OPUSDecoder_AllocateBuffers()) {
                AUDIO_ERROR("The OPUSDecoder could not be initialized");
                goto exit;
            }
        ...
        case CODEC_VORBIS:
            if(!VORBISDecoder_AllocateBuffers()) {
                AUDIO_ERROR("The VORBISDecoder could not be initialized");
                goto exit;
            }
...
int Audio::findNextSync(uint8_t* data, size_t len) {
    ...
    if(m_fnsy.nextSync == -1) {
        ...
        else {m_fnsy.swnf++;} // syncword not found counter, can be multimediadata
    }
...
void Audio::setDecoderItems() {
    ...
    //    setBitrate(MP3GetBitrate());
        if(!m_nominal_bitrate) m_nominal_bitrate = MP3GetBitrate();
    ...
    //    setBitrate(AACGetBitrate());
        if(!m_nominal_bitrate) m_nominal_bitrate = AACGetBitrate();
    }
    if(m_codec == CODEC_FLAC) {
    ...
    //    setBitrate(FLACGetBitRate());
        if(!m_nominal_bitrate) m_nominal_bitrate = FLACGetBitRate();
    ...
    //    setBitrate(OPUSGetBitRate());
        if(!m_nominal_bitrate) m_nominal_bitrate = OPUSGetBitRate();
...
//            info(evt_bitrate, "%i", m_nominal_bitrate);
            AUDIO_INFO("Duration: %lu (s)", m_audioFileDuration);
            AUDIO_INFO("Bitrate: %lu", m_nominal_bitrate);
        }*/
    }
    if(m_codec == CODEC_VORBIS) {
    ...
    //    setBitrate(VORBISGetBitRate());
        if(!m_nominal_bitrate) m_nominal_bitrate = VORBISGetBitRate();
...
/*                            vec = FLACgetMetadataBlockPicture();
                            if(vec.size() > 0){ // get blockpic data
                                // AUDIO_INFO("---------------------------------------------------------------------------");
                                // AUDIO_INFO("ogg metadata blockpicture found:");
                                // for(int i = 0; i < vec.size(); i += 2) { AUDIO_INFO("segment %02i, pos %07i, len %05i", i / 2, vec[i], vec[i + 1]); }
                                // AUDIO_INFO("---------------------------------------------------------------------------");
                                info(evt_image, vec);
                            }*/
                            break;
...
bool Audio::setSampleRate(uint32_t sampRate) {
    ...
    if(sampRate < 8000 ) {
        /*AUDIO_LOG_WARN*/AUDIO_INFO("Sample rate must not be smaller than 8kHz, found: %lu", sampRate);
        m_sampleRate = 8000;
    }
...
void Audio::IIR_calculateCoefficients(int8_t G0, int8_t G1, int8_t G2) { // Infinite Impulse Response (IIR) filters
    ...
    //    AUDIO_INFO("LS a0=%f, a1=%f, a2=%f, b1=%f, b2=%f", m_filter[0].a0, m_filter[0].a1, m_filter[0].a2,
    //                                                  m_filter[0].b1, m_filter[0].b2);
    //    AUDIO_INFO("EQ a0=%f, a1=%f, a2=%f, b1=%f, b2=%f", m_filter[1].a0, m_filter[1].a1, m_filter[1].a2,
    //                                                  m_filter[1].b1, m_filter[1].b2);
    //    AUDIO_INFO("HS a0=%f, a1=%f, a2=%f, b1=%f, b2=%f", m_filter[2].a0, m_filter[2].a1, m_filter[2].a2,
