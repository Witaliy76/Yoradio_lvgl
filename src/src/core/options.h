...
#define YOVERSION "0.9.434m-r2-lvgl-beta.2-fu4.2.12"
 ...
 * ESP32-S3: only RTC GPIOs 0-21 work with EXT0. Use an external pull; do not float the line.
 * GPIO0 + WAKE_LEVEL=LOW held through reset can force download mode — avoid that pairing.
 ...
 * ESP32-S3: только RTC GPIO 0-21 для EXT0. Нужна внешняя подтяжка, линия не должна висеть.
 * GPIO0 + WAKE_LEVEL=LOW, удержанный через reset, может включить download mode — не использовать. */
#ifndef WAKE_PIN
  #ifdef WAKEUP_PIN
