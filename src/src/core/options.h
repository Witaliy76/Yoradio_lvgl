...
#define YOVERSION "0.9.434m-r2-lvgl-beta.2-fu4.2.22"
...
#endif
// E5A: pre-Gain PCM telemetry (Serial diagnostic). Default OFF; enable in myoptions.h for soak.
// E5A: телеметрия PCM до Gain (Serial diagnostic). По умолчанию OFF; включать в myoptions.h.
...
/*        BUTTONS                 */
  ...
  #define BTN_MODE             255
#endif
/* Physical MUTE button (momentary, active-low, same electrical contract as the other BTN_xxx).
 * NOTE: unrelated to MUTE_PIN below - that one is an amplifier-enable OUTPUT, not an input.
 * Физическая кнопка MUTE (замыкание на GND, тот же контракт, что и остальные BTN_xxx).
 * ВНИМАНИЕ: не путать с MUTE_PIN ниже - это ВЫХОД управления усилителем, а не вход. */
#ifndef BTN_MUTE
  #define BTN_MUTE             255
#endif
...
#endif
#ifndef MUTE_VAL
  #define MUTE_VAL      HIGH  // Asserted (muted) level: written to MUTE_PIN when the radio is
                              // not audible - stopped, connecting, or semantically muted
#endif
#ifndef MUTE_LOCK
  #define MUTE_LOCK      false  // for vs1053 Shield rev2 - lock MUTE_PIN to !MUTE_VAL.
                                // When true the line never asserts, so semantic MUTE
                                // (BTN_MUTE / IR / WebUI) stays codec-only on that board.
#endif
...
#endif
/* Deep-sleep GPIO wake (compile-time board option, not a Settings control).
 * 255 disables the source; the pin is not claimed at runtime and is not a UI button.
 * ESP32-S3: only RTC GPIOs 0-21 work with EXT0. The line needs a stable pull and must not float -
 * either an external resistor or an on-board button that already provides one (BOOT on GPIO0).
 * GPIO0 is a valid EXT0 wake source, but it is also a boot strapping pin: it must not be held
 * LOW across a hardware Reset, or the chip enters download mode instead of booting.
 * Пробуждение из deep sleep по GPIO (опция платы, не пункт Settings).
 * 255 выключает источник; GPIO не занимается в runtime и не становится кнопкой UI.
 * ESP32-S3: только RTC GPIO 0-21 для EXT0. Нужна стабильная подтяжка, линия не должна висеть:
 * внешний резистор либо штатная кнопка платы со своей подтяжкой (BOOT на GPIO0).
 * GPIO0 допустим как источник EXT0, но это ещё и boot strapping-пин: его нельзя удерживать
 * LOW во время аппаратного Reset, иначе чип уйдёт в download mode вместо загрузки. */
#ifndef WAKE_PIN
  #ifdef WAKEUP_PIN
    #define WAKE_PIN            WAKEUP_PIN
  #else
    #define WAKE_PIN            255
  #endif
#endif
#ifndef WAKE_LEVEL
  #ifdef WAKEUP_LEVEL
    #define WAKE_LEVEL          WAKEUP_LEVEL
  #else
    #define WAKE_LEVEL          LOW   // historical/default: wake when the pin goes LOW
  #endif
#endif
#if defined(CONFIG_IDF_TARGET_ESP32S3)
  #if (WAKE_PIN != 255) && ((WAKE_PIN < 0) || (WAKE_PIN > 21))
    #error WAKE_PIN must be an ESP32-S3 RTC GPIO (0-21), or 255 to disable
  #endif
#endif
#if (WAKE_LEVEL != LOW) && (WAKE_LEVEL != HIGH)
  #error WAKE_LEVEL must be LOW or HIGH
#endif
#ifndef LIGHT_SENSOR
