...
// Wi-Fi 5E: one-line finish log (no SSID/password) / один лог итога без SSID/пароля.
static void wifiops_log_connect_finish(WifiOpsResult r) {
  Serial.printf("[WiFiOps] connect finish: result=%u, status=%d\n",
                static_cast<unsigned>(static_cast<uint8_t>(r)),
...
static WifiOpsResult attempt_sta_connect(const char* ssid,
  ...
  // Wi-Fi 5E: boot-fail Recovery keeps legacy WIFI_AP; STA needs AP_STA, not WIFI_STA-only / AP + STA без гашения AP.
  if (WiFi.getMode() == WIFI_AP) {
    const wifi_mode_t mode_before = WiFi.getMode();
    Serial.printf("[WiFiOps] AP mode detected, switching to AP_STA (mode_before=%u)\n",
                  static_cast<unsigned>(mode_before));
    WiFi.mode(WIFI_AP_STA);
    vTaskDelay(pdMS_TO_TICKS(160));
    Serial.printf("[WiFiOps] mode after policy: mode=%u\n", static_cast<unsigned>(WiFi.getMode()));
  }

  Serial.printf("[WiFiOps] connect begin: mode=%u, status=%d\n",
                static_cast<unsigned>(WiFi.getMode()),
                static_cast<int>(WiFi.status()));

  if (ssid == nullptr || ssid[0] == '\0') {
    const WifiOpsResult r = WifiOpsResult::InternalError;
    wifiops_log_connect_finish(r);
    return r;
  }
  if (guard_already_connected && WiFi.status() == WL_CONNECTED && !allow_disc) {
    const WifiOpsResult r = WifiOpsResult::AlreadyConnected;
    wifiops_log_connect_finish(r);
    return r;
  }
  ...
  Serial.printf("[WiFiOps] WiFi.begin rc=%d\n", static_cast<int>(begin_rc));
  ...
  for (;;) {
    if (s_cancel_requested.load(std::memory_order_acquire)) {
      ...
      WiFi.setAutoReconnect(prev_auto);
      const WifiOpsResult r = WifiOpsResult::Cancelled;
      wifiops_log_connect_finish(r);
      return r;
    }
    ...
    if (st == WL_CONNECTED) {
      WiFi.setAutoReconnect(prev_auto);
      const WifiOpsResult r = WifiOpsResult::Success;
      wifiops_log_connect_finish(r);
      return r;
    }
    if (st == WL_CONNECT_FAILED) {
      ...
      WiFi.setAutoReconnect(prev_auto);
      const WifiOpsResult r = WifiOpsResult::AuthFailed;
      wifiops_log_connect_finish(r);
      return r;
    }
    if (st == WL_NO_SSID_AVAIL) {
      ...
      vTaskDelay(pdMS_TO_TICKS(50));
      WiFi.setAutoReconnect(prev_auto);
      const WifiOpsResult r = WifiOpsResult::NoNetwork;
      wifiops_log_connect_finish(r);
      return r;
    }
    if ((int32_t)(millis() - deadline) >= 0) {
      ...
      WiFi.setAutoReconnect(prev_auto);
      const WifiOpsResult r = WifiOpsResult::Timeout;
      wifiops_log_connect_finish(r);
      return r;
    }
    vTaskDelay(pdMS_TO_TICKS(75));
...
static void run_connect_job(const char* ssid, const char* pass, bool allow_disc) {
  ...
  // Always publish finish - short timeouts could leave UI stuck on Connecting / всегда фиксируем итог, иначе UI зависает.
...
static void run_try_saved_job(bool allow_disc) {
  // Wi-Fi 5E: manual connect uses AP_STA from WIFI_AP; try-saved still deferred (AP-only) / try-saved — follow-up.
  if (WiFi.getMode() == WIFI_AP) {
    if (take_mutex(pdMS_TO_TICKS(50))) {
